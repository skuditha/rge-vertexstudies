...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fit local LD2 + solid peaks for ld2_solid runs.")
    ...
    parser.add_argument("--polarity", default=None, help="Optional single polarity filter.")
    ...
    fit_results_csv = repo_path(output_cfg.get("fit_results_csv", "outputs/fits/ld2_solid_local_fit_results.csv"), repo_root)
    unresolved_csv = repo_path(output_cfg.get("unresolved_csv", "outputs/tables/unresolved_ld2_solid_categories.csv"), repo_root)
    category_summary_csv = repo_path(output_cfg.get("category_summary_csv", "outputs/tables/ld2_solid_category_summary.csv"), repo_root)
    ...
    for run, meta in selected_runs:
        ...
        print(
            ...
            f"solid_target={meta.get('solid_target')}, output={root_path}"
        )
    ...
    save_ld2_solid_local_outputs(
        ...
        category_summary_csv=category_summary_csv,
    )

    print(f"\nWrote fit results: {fit_results_csv}")
    print(f"Wrote unresolved categories: {unresolved_csv}")
